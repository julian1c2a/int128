...
void writeCSV(const std::vector<BenchmarkResult> &results, const std::string &filename)
{
    ...
    {
        file << result.operation << "," << result.time_ns << ","
             << result.iterations << "," << result.ops_per_second << "\n";
    }
}
...
int main()
{
    ...
    // Addition
    arithmetic_results.push_back(benchmark("Addition", [&]()
                                           { volatile auto result = a + b; }));

    // Subtraction
    arithmetic_results.push_back(benchmark("Subtraction", [&]()
                                           { volatile auto result = a - b; }));

    // Multiplication
    arithmetic_results.push_back(benchmark("Multiplication", [&]()
                                           { volatile auto result = a * b; }, 100000)); // Fewer iterations for slower operations

    // Bitwise AND
    arithmetic_results.push_back(benchmark("Bitwise_AND", [&]()
                                           { volatile auto result = a & b; }));

    // Left shift
    arithmetic_results.push_back(benchmark("Left_Shift", [&]()
                                           { volatile auto result = a << 1; }));

    std::cout << "Running comparison benchmarks...\n";

    // Equality
    comparison_results.push_back(benchmark("Equality", [&]()
                                           { volatile bool result = a == b; }));

    // Less than
    comparison_results.push_back(benchmark("Less_Than", [&]()
                                           { volatile bool result = a < b; }));
    ...
    for (const auto &result : arithmetic_results)
    {
        std::cout << result.operation << "\t\t"
                  << std::fixed << std::setprecision(2) << result.time_ns
