...
int main()
{
    ...
    std::cout << "[OK] std::common_type<uint128_t, int128_t> está definido!\n";
    ...
    std::cout << "[OK] std::common_type<int128_t, uint128_t> está definido!\n";
    std::cout << "   Tipo resultante: "
    ...
    if (std::is_same_v<CommonType, CommonType2>) {
        std::cout << "[OK] Ambas especializaciones dan el mismo tipo (int128_t)\n";
    }
    ...
    std::cout << "[OK] Operación mixta: uint128_t(100) + int128_t(-50) = " << result.to_string()
              << "\n";
