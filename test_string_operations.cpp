...
void test_constructor_from_cstr()
{
    ...
    std::cout << "  12345 (decimal) -> " << u1.to_string() << " ✓\n";

    // Hexadecimal (prefijo 0x)
    uint128_t u2("0xFF");
    assert(u2 == uint128_t(0ull, 255ull));
    std::cout << "  0xFF (hex) -> " << u2.to_string() << " ✓\n";

    // Binario (prefijo 0b)
    uint128_t u3("0b11111111");
    assert(u3 == uint128_t(0ull, 255ull));
    std::cout << "  0b11111111 (bin) -> " << u3.to_string() << " ✓\n";

    // Octal (prefijo 0)
    uint128_t u4("0377");
    assert(u4 == uint128_t(0ull, 255ull));
    std::cout << "  0377 (oct) -> " << u4.to_string() << " ✓\n";

    // Signed positivo
    int128_t i1("42");
    assert(i1 == int128_t(0ull, 42ull));
    std::cout << "  42 (signed positivo) -> " << i1.to_string() << " ✓\n";

    // Signed negativo
    int128_t i2("-42");
    assert(i2.is_negative());
    std::cout << "  -42 (signed negativo) -> " << i2.to_string() << " ✓\n";
...
void test_constructor_from_cstr_with_base()
{
    ...
    std::cout << "  11111111 (base 2) -> " << u1.to_string() << " ✓\n";

    // Base 8 (octal)
    uint128_t u2("377", 8);
    assert(u2 == uint128_t(0ull, 255ull));
    std::cout << "  377 (base 8) -> " << u2.to_string() << " ✓\n";

    // Base 16 (hexadecimal)
    uint128_t u3("FF", 16);
    assert(u3 == uint128_t(0ull, 255ull));
    std::cout << "  FF (base 16) -> " << u3.to_string() << " ✓\n";

    // Base 36 (máximo)
    uint128_t u4("ZZ", 36);
    std::cout << "  ZZ (base 36) -> " << u4.to_string() << " ✓\n";
...
void test_constructor_from_string()
{
    ...
    uint128_t u1(s1);
    assert(u1 == uint128_t(0ull, 12345ull));
    std::cout << "  std::string(\"12345\") -> " << u1.to_string() << " ✓\n";

    std::string s2 = "0xDEADBEEF";
    uint128_t u2(s2);
    std::cout << "  std::string(\"0xDEADBEEF\") -> " << u2.to_string() << " ✓\n";

    std::string s3 = "-999";
    int128_t i1(s3);
    assert(i1.is_negative());
    std::cout << "  std::string(\"-999\") -> " << i1.to_string() << " ✓\n";
...
void test_constructor_from_string_view()
{
    ...
    std::cout << "  std::string_view(\"67890\") -> " << u1.to_string() << " ✓\n";

    std::string_view sv2 = "0xABCD";
    uint128_t u2(sv2);
    std::cout << "  std::string_view(\"0xABCD\") -> " << u2.to_string() << " ✓\n";

    std::string_view sv3 = "-123";
    int128_t i1(sv3);
    assert(i1.is_negative());
    std::cout << "  std::string_view(\"-123\") -> " << i1.to_string() << " ✓\n";
...
void test_assignment_from_cstr()
{
    ...
    std::cout << "  u1 = \"99999\" -> " << u1.to_string() << " ✓\n";

    u1 = "0xCAFE";
    std::cout << "  u1 = \"0xCAFE\" -> " << u1.to_string() << " ✓\n";

    int128_t i1;
    i1 = "-777";
    assert(i1.is_negative());
    std::cout << "  i1 = \"-777\" -> " << i1.to_string() << " ✓\n";
...
void test_stream_output()
{
    ...
    std::cout << "  uint128_t(12345) -> \"" << oss.str() << "\" ✓\n";

    oss.str("");
    int128_t i1(0ull, 999ull);
    oss << i1;
    assert(oss.str() == "999");
    std::cout << "  int128_t(999) -> \"" << oss.str() << "\" ✓\n";
...
void test_stream_input()
{
    ...
    std::cout << "  stream(\"54321\") -> uint128_t(" << u1.to_string() << ") ✓\n";

    iss.clear();
    iss.str("0xABCD");
    uint128_t u2;
    iss >> u2;
    std::cout << "  stream(\"0xABCD\") -> uint128_t(" << u2.to_string() << ") ✓\n";
    ...
    iss >> i1;
    assert(i1.is_negative());
    std::cout << "  stream(\"-777\") -> int128_t(" << i1.to_string() << ") ✓\n";
...
void test_error_handling()
{
    ...
    try {
        ...
        std::cout << "  nullptr -> excepcion: \"" << e.what() << "\" ✓\n";
    }
    ...
    try {
        ...
        std::cout << "  \"123abc\" -> excepcion: \"" << e.what() << "\" ✓\n";
    }
    ...
    try {
        ...
        std::cout << "  base 1 -> excepcion: \"" << e.what() << "\" ✓\n";
    }
    ...
    try {
        ...
        std::cout << "  to_string(37) -> excepcion: \"" << e.what() << "\" ✓\n";
    }
...
void test_roundtrip()
{
    ...
    assert(u2 == u3);
    std::cout << "  0xDEADBEEFCAFE -> \"" << hex << "\" -> uint128_t ✓\n";
    ...
    int128_t i2(si);
    assert(i1 == i2);
    std::cout << "  \"-987654321\" -> int128_t -> \"" << si << "\" ✓\n";
