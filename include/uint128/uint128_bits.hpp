                                ...
                                inline int countr_one(const uint128_t& value) noexcept
                                {
                                    if (value.low() == UINT64_MAX) {
                                        ...
                                        inline
                                             ...
                                             */
                                            constexpr bool
                                            has_single_bit(const uint128_t& value) noexcept
                                        {
                                            return value != uint128_t(0) && popcount(value) == 1;
                                        }
                                         ...
                                         * @brief Find the largest power of 2 not greater than value
                                         */
                                        constexpr uint128_t bit_floor(
                                            const uint128_t& value) noexcept
                                        {
                                            if (value == uint128_t(0)) {
                                                return uint128_t(0);
                                            }
                                     ...
                                     * @brief Rotate left
                                     */
                                    constexpr uint128_t rotl(const uint128_t& value,
                                                             int shift) noexcept
                                    {
                                        shift %= 128;
                                        if (shift == 0)
                                     ...
                                     * @brief Rotate right
                                     */
                                    constexpr uint128_t rotr(const uint128_t& value,
                                                             int shift) noexcept
                                    {
                                        shift %= 128;
                                        if (shift == 0)
                                     ...
                                     */
                                    constexpr uint128_t
                                    reverse_bits(const uint128_t& value) noexcept
                                    {
                                        ...
                                        auto reverse64 = [](uint64_t v) -> uint64_t {
                                            ...
                                            inline uint128_t byteswap(
                                                const uint128_t& value) noexcept
                                            {
#ifdef _MSC_VER
                                                return uint128_t(_byteswap_uint64(value.low()),
                                                                 _byteswap_uint64(value.high()));
#else
                                                auto swap64 = [](uint64_t v) -> uint64_t {
                                                    return __builtin_bswap64(v);
                                                };
                                                return uint128_t(swap64(value.low()),
                                                                 swap64(value.high()));
#endif

                                                return uint128_t(reverse64(value.low()),
                                                                 reverse64(value.high()));
                                            }

                                            /**
                                             * @brief Reverse bytes (endianness swap)
                                             */
                                            constexpr uint128_t byteswap(
                                                const uint128_t& value) noexcept
                                            {
                                                auto swap64 = [](uint64_t v) -> uint64_t {
                                                    return __builtin_bswap64(v);
                                             ...
                                             */
                                            constexpr uint128_t extract_bits(const uint128_t& value,
                                                                             int offset,
                                                                             int width) noexcept
                                            {
                                             ...
                                             */
                                            constexpr uint128_t insert_bits(
                                                const uint128_t& dest, const uint128_t& src,
                                                int offset, int width) noexcept
                                            {
                                                if (width <= 0 || offset < 0 || offset >= 128) {
                                                    return dest;
                                                }
                                             ...
                                             */
                                            constexpr uint128_t parallel_deposit(
                                                const uint128_t& value,
                                                const uint128_t& mask) noexcept
                                            {
                                                uint128_t result(0);
                                                uint128_t src_bit(1);
                                             ...
                                             */
                                            constexpr uint128_t parallel_extract(
                                                const uint128_t& value,
                                                const uint128_t& mask) noexcept
                                            {
                                                uint128_t result(0);
                                             ...
                                             */
                                            constexpr bool get_bit(const uint128_t& value,
                                             ...
                                             */
                                            constexpr uint128_t set_bit(
                                                const uint128_t& value, int position,
                                             ...
                                             */
                                            constexpr uint128_t flip_bit(const uint128_t& value,
                                                                         int position) noexcept
                                            {
                                                if (position < 0 || position >= 128) {
                                                    return value;
                                        ...
                                        namespace std
                                        {
                                             ...
                                             */
                                            inline std::bitset<128> to_bitset(
                                                const uint128_t& value) noexcept
                                            {
                                             ...
                                             */
                                            template <size_t N>
                                            inline uint128_t from_bitset(
                                                const std::bitset<N>& bs) noexcept
                                            {
                                                static_assert(N <= 128,
                                                              "Bitset size cannot exceed 128 bits");

                                                uint64_t low_part = 0;
                                                uint64_t high_part = 0;

                                                // Extract low 64 bits
                                                for (int i = 0; i < 64 && i < static_cast<int>(N);
                                                     ++i) {
                                                    if (bs[i]) {
                                                        low_part |= (1ULL << i);
                                                    }
                                                ...
                                                for (int i = 64; i < 128 && i < static_cast<int>(N);
                                                     ++i) {
                                                    if (bs[i]) {
                                                        high_part |= (1ULL << (i - 64));
                                                    }
