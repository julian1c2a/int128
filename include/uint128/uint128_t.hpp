...
#pragma warning(pop)
                    const unsigned char borrow =
                        _subborrow_u64(0, data[0], other.data[0], &new_low);
                    data[0] = new_low;
...
#pragma warning(push)
#pragma warning(disable : 4701)
                    [[maybe_unused]] unsigned long
                        index; // NOLINT(cppcoreguidelines-init-variables)
#pragma warning(pop)
            ...
            constexpr uint128_t shift_left(int positions) const noexcept
            {
                ...
                if (positions >= 64) {
                    ...
                    const uint64_t new_high =
                        (data[1] << positions) | (data[0] >> (64 - positions));
          ...
          private:
            ...
            constexpr std::pair<uint128_t, int> normalize_divisor(const uint128_t& dividendo)
                const noexcept
            {
          ...
          public:
             ...
             * @endcode
             */
            constexpr std::optional<std::pair<uint128_t, uint128_t>> divrem(
             ...
             */
            template <typename T>
            constexpr std::optional<std::pair<uint128_t, uint128_t>> divrem(T divisor)
                const noexcept
            {
                static_assert(std::is_integral<T>::value, "T must be an integral type");
        ...
        return p3 + (middle >> 32) + (p2 >> 32);
            ...
            std::optional<std::pair<uint128_t, uint128_t>> try_divide_by_power_of_10(
                const uint128_t& divisor) const noexcept
            {
            ...
            std::optional<std::pair<uint128_t, uint128_t>> try_divide_by_power_of_3(
                const uint128_t& divisor) const noexcept
            {
            ...
            std::optional<std::pair<uint128_t, uint128_t>> try_divide_by_power_of_5(
                const uint128_t& divisor) const noexcept
            {
        ...
        return high_high + carry;
             ...
             */
            std::optional<std::pair<uint128_t, uint128_t>> knuth_D_divrem(const uint128_t& v_in)
                const noexcept
            {
                // 0. Casos triviales
                ...
                if (v_in.data[1] == 0) {
                    ...
                    const uint128_t quotient(static_cast<uint64_t>(q >> 64),
