#!/usr/bin/env bash
# ==============================================================================
# build_int128_thread_safety_extracted_tests.bash
# ==============================================================================
# Compiles int128 thread_safety tests with all 4 compilers (GCC, Clang, Intel, MSVC)
# Output: build/build_tests/{compiler}/{debug|release}/int128_thread_safety_tests_{compiler}[.exe]
# ==============================================================================
...
echo_info "Building int128 thread_safety tests for all compilers..."
echo ""
...
for MODE in debug release; do
    ...
    if $GCC_CXX $COMMON_FLAGS $MODE_FLAGS "$TEST_FILE" -o "$OUTPUT" -latomic 2>&1; then
        if [[ -f "$OUTPUT" ]]; then
...
if ! command -v "$CLANG_CXX" &>/dev/null; then
    ...
    for MODE in debug release; do
        ...
        if $CLANG_CXX $COMMON_FLAGS $MODE_FLAGS "$TEST_FILE" -o "$OUTPUT" -latomic 2>&1; then
            if [[ -f "$OUTPUT" ]]; then
...
echo ""
echo_success "Build complete for int128 thread_safety tests!"
