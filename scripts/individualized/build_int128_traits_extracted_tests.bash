#!/bin/bash
# Script para compilar tests de int128_traits
# Uso: ./build_int128_traits_extracted_tests.bash [gcc|clang|intel|msvc|all] [debug|release]
...
echo -e "${BLUE}============================================================${NC}"
echo -e "${BLUE}  Compilación de int128_traits tests${NC}"
...
# -----------------------------------------------------------------------------
build_gcc() {
    echo -e "\n${YELLOW}[GCC] Compilando tests...${NC}"
    ...
    local OUTPUT="build/build_tests/gcc/${MODE}/int128_traits_tests_gcc"
    
    g++ $FLAGS "${TEST_SRC}" -o "${OUTPUT}"
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}    [OK] Tests compilados OK${NC}"
        ls -lh --time-style='+%Y-%m-%d %H:%M:%S' "${OUTPUT}" | awk -v path="${OUTPUT}" '{print "    Ejecutable:", $6, $7, "-", path, "(" $5 ")"}'
        return 0
    else
        echo -e "${RED}    [FAIL] Error compilando tests${NC}"
        return 1
    fi
}
...
# -----------------------------------------------------------------------------
build_clang() {
    echo -e "\n${YELLOW}[CLANG] Compilando tests...${NC}"
    ...
    local OUTPUT="build/build_tests/clang/${MODE}/int128_traits_tests_clang"
...
# -----------------------------------------------------------------------------
build_intel() {
    echo -e "\n${YELLOW}[INTEL] Compilando tests con Intel OneAPI...${NC}"
    ...
    local OUTPUT="build/build_tests/intel/${MODE}/int128_traits_tests_intel.exe"
    ...
    icx $FLAGS \
        ...
        -o build/build_tests/intel/${MODE}/int128_traits_tests_intel.exe
...
# -----------------------------------------------------------------------------
build_msvc() {
    echo -e "\n${YELLOW}[MSVC] Compilando tests...${NC}"
    ...
    local OUTPUT="build/build_tests/msvc/${MODE}/int128_traits_tests_msvc.exe"
    ...
    if [ $result -eq 0 ]; then
        ...
        rm -f build/build_tests/msvc/${MODE}/*.obj build/build_tests/msvc/${MODE}/*.pdb
        ls -lh --time-style='+%Y-%m-%d %H:%M:%S' "${OUTPUT}" | awk -v path="${OUTPUT}" '{print "    Ejecutable:", $6, $7, "-", path, "(" $5 ")"}'
        return 0
    else
        echo -e "${RED}    [FAIL] Error compilando tests${NC}"
