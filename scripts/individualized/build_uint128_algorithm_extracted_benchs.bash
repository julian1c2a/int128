#!/usr/bin/env bash
# ==============================================================================
# build_uint128_algorithm_extracted_benchs.bash
# ==============================================================================
# Compiles uint128 algorithm benchmarks with all 4 compilers (GCC, Clang, Intel, MSVC)
# Output: build/build_benchmarks/{compiler}/{debug|release}/uint128_algorithm_benchmarks_{compiler}[.exe]
# ==============================================================================
...
echo_info "Building uint128 algorithm benchmarks for all compilers..."
echo ""
...
for MODE in debug release; do
    ...
    if $GCC_CXX $COMMON_FLAGS $MODE_FLAGS "$BENCH_FILE" -o "$OUTPUT" 2>&1; then
        if [[ -f "$OUTPUT" ]]; then
...
if ! command -v "$CLANG_CXX" &>/dev/null; then
    ...
    for MODE in debug release; do
        ...
        if $CLANG_CXX $COMMON_FLAGS $MODE_FLAGS "$BENCH_FILE" -o "$OUTPUT" 2>&1; then
            if [[ -f "$OUTPUT" ]]; then
...
echo ""
echo_success "Build complete for uint128 algorithm benchmarks!"
