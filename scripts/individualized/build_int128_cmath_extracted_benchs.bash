...
if [ "$1" = "gcc" ] || [ "$1" = "all" ] || [ -z "$1" ]; then
    ...
    if ! command -v g++ &> /dev/null; then
        ...
        if [ $result_debug -eq 0 ] && [ $result_release -eq 0 ]; then
            echo "   [OK] GCC Debug: build/build_benchmarks/gcc/debug/int128_cmath_benchmarks_gcc"
            echo "   [OK] GCC Release: build/build_benchmarks/gcc/release/int128_cmath_benchmarks_gcc"
...
if [ "$1" = "clang" ] || [ "$1" = "all" ] || [ -z "$1" ]; then
    ...
    if ! command -v clang++ &> /dev/null; then
        ...
        if [ $result_debug -eq 0 ] && [ $result_release -eq 0 ]; then
            echo "   [OK] Clang Debug: build/build_benchmarks/clang/debug/int128_cmath_benchmarks_clang"
            echo "   [OK] Clang Release: build/build_benchmarks/clang/release/int128_cmath_benchmarks_clang"
        elif [ $result_debug -eq 0 ]; then
            echo "   [OK] Clang Debug: build/build_benchmarks/clang/debug/int128_cmath_benchmarks_clang"
...
if [ "$1" = "intel" ] || [ "$1" = "all" ] || [ -z "$1" ]; then
    ...
    if ! command -v icpx &> /dev/null; then
        ...
        if [ $result_debug -eq 0 ] && [ $result_release -eq 0 ]; then
            echo "   [OK] Intel Debug: build/build_benchmarks/intel/debug/int128_cmath_benchmarks_intel"
            echo "   [OK] Intel Release: build/build_benchmarks/intel/release/int128_cmath_benchmarks_intel"
        elif [ $result_debug -eq 0 ]; then
            echo "   [OK] Intel Debug: build/build_benchmarks/intel/debug/int128_cmath_benchmarks_intel"
...
if [ "$1" = "msvc" ] || [ "$1" = "all" ] || [ -z "$1" ]; then
    ...
    if ! command -v cl.exe &> /dev/null; then
        ...
        if [ $result_debug -eq 0 ] && [ $result_release -eq 0 ]; then
            echo "   [OK] MSVC Debug: build/build_benchmarks/msvc/debug/int128_cmath_benchmarks_msvc.exe"
            echo "   [OK] MSVC Release: build/build_benchmarks/msvc/release/int128_cmath_benchmarks_msvc.exe"
        elif [ $result_debug -eq 0 ]; then
            echo "   [OK] MSVC Debug: build/build_benchmarks/msvc/debug/int128_cmath_benchmarks_msvc.exe"
