#!/usr/bin/env bash
# ==============================================================================
# check_uint128_iostreams_extracted_tests.bash
# ==============================================================================
# Executes uint128 iostreams test binaries across all compilers and modes
...
TYPE="uint128"
FEATURE="safe"
TEST_SOURCE="test_iostreams_functions"
...
echo_header "========================================================================"
echo_header "  UINT128 iostreams TESTS - Execution"
echo_header "========================================================================"
echo ""
...
for compiler in gcc clang intel msvc; do
    ...
    if [[ "$debug_result" == "PASS" ]]; then
        debug_display="${GREEN}[OK] PASS${NC}"
    elif [[ "$debug_result" == "FAIL" ]]; then
        debug_display="${RED}[ERROR] FAIL${NC}"
    else
        debug_display="${YELLOW}⊘ SKIP${NC}"
    fi
    
    if [[ "$release_result" == "PASS" ]]; then
        release_display="${GREEN}[OK] PASS${NC}"
    elif [[ "$release_result" == "FAIL" ]]; then
        release_display="${RED}[ERROR] FAIL${NC}"
    else
