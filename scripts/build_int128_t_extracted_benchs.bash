#!/usr/bin/env bash
# Build script para int128_t_extracted_benchs.cpp con 4 compiladores
...
# Mostrar timestamp
echo "========================================="
echo " BUILD: int128_t_extracted_benchs"
...
# Archivo fuente
SOURCE_FILE="$PROJECT_ROOT/benchs/int128_t_extracted_benchs.cpp"
    ...
    if ! command -v g++ &> /dev/null; then
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "debug" ]; then
            ...
            g++ "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -g -O0 \
                -o "$PROJECT_ROOT/build/build_benchs/gcc/debug/int128_t_extracted_benchs.exe"
            echo "   ✅ GCC Debug OK"
        fi
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "release" ]; then
            ...
            g++ "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -O3 -DNDEBUG \
                -o "$PROJECT_ROOT/build/build_benchs/gcc/release/int128_t_extracted_benchs.exe"
            echo "   ✅ GCC Release OK"
        fi
    fi
fi
    ...
    if ! command -v "$CLANG_BIN" &> /dev/null && ! [ -f "$CLANG_BIN" ]; then
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "debug" ]; then
            ...
            "$CLANG_BIN" "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -g -O0 \
                -o "$PROJECT_ROOT/build/build_benchs/clang/debug/int128_t_extracted_benchs.exe" 2>&1 | grep -a -v "LINK : warning LNK" | grep -a -v "^LINK : " || true
            if [ -f "$PROJECT_ROOT/build/build_benchs/clang/debug/int128_t_extracted_benchs.exe" ]; then
                echo "   ✅ Clang Debug OK"
            else
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "release" ]; then
            ...
            "$CLANG_BIN" "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -O3 -DNDEBUG \
                -o "$PROJECT_ROOT/build/build_benchs/clang/release/int128_t_extracted_benchs.exe" 2>&1 | grep -a -v "LINK : warning LNK" | grep -a -v "^LINK : " || true
            if [ -f "$PROJECT_ROOT/build/build_benchs/clang/release/int128_t_extracted_benchs.exe" ]; then
...
if [ "$COMPILER" = "all" ] || [ "$COMPILER" = "intel" ]; then
    ...
    if ! command -v "$INTEL_CMD" &> /dev/null; then
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "debug" ]; then
            ...
            WIN_OUT_DEBUG=$(cygpath -m "$PROJECT_ROOT/build/build_benchs/intel/debug/int128_t_extracted_benchs.exe")
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "release" ]; then
            ...
            WIN_OUT_RELEASE=$(cygpath -m "$PROJECT_ROOT/build/build_benchs/intel/release/int128_t_extracted_benchs.exe")
...
if [ "$COMPILER" = "all" ] || [ "$COMPILER" = "msvc" ]; then
    ...
    if ! command -v cl.exe &> /dev/null; then
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "debug" ]; then
            ...
            WIN_OUT_DEBUG=$(cygpath -m "$PROJECT_ROOT/build/build_benchs/msvc/debug/int128_t_extracted_benchs.exe")
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "release" ]; then
            ...
            WIN_OUT_RELEASE=$(cygpath -m "$PROJECT_ROOT/build/build_benchs/msvc/release/int128_t_extracted_benchs.exe")
