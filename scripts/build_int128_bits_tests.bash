#!/usr/bin/env bash
# Script para compilar int128_bits_tests.cpp con diferentes compiladores
...
SOURCE_FILE="${PROJECT_ROOT}/tests/int128_bits_tests.cpp"
...
echo -e "${COLOR_BLUE}  Build int128_bits_tests${COLOR_RESET}"
echo -e "${COLOR_BLUE}========================================${COLOR_RESET}"
...
build_gcc() {
    ...
    if g++ ${flags} "${SOURCE_FILE}" -o "${output_dir}/int128_bits_tests.exe"; then
        echo -e "${COLOR_GREEN}[OK] GCC compilacion exitosa${COLOR_RESET}"
        echo -e "${COLOR_GREEN}     Output: ${output_dir}/int128_bits_tests.exe${COLOR_RESET}"
        return 0
    else
...
build_clang() {
    ...
    if clang++ ${flags} "${SOURCE_FILE}" -o "${output_dir}/int128_bits_tests.exe"; then
...
build_msvc() {
    ...
    local win_output_file=$(cygpath -m "${output_dir}/int128_bits_tests.exe")
