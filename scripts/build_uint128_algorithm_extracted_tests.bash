#!/usr/bin/env bash
# ==============================================================================
# build_uint128_algorithm_extracted_tests.bash
# ==============================================================================
# Compiles uint128 algorithm tests with all 4 compilers (GCC, Clang, Intel, MSVC)
# Output: build/build_tests/{compiler}/{debug|release}/uint128_algorithm_tests_{compiler}[.exe]
# ==============================================================================
...
TYPE="uint128"
FEATURE="algorithm"
TEST_FILE="tests/uint128_algorithm_extracted_tests.cpp"
...
echo_info "Building uint128 algorithm tests for all compilers..."
echo ""
...
for MODE in debug release; do
    ...
    OUTPUT="$OUTPUT_DIR/${TYPE}_${FEATURE}_tests_${COMPILER}"
    ...
    if $GCC_CXX $COMMON_FLAGS $MODE_FLAGS "$TEST_FILE" -o "$OUTPUT" 2>&1; then
        if [[ -f "$OUTPUT" ]]; then
...
if ! command -v "$CLANG_CXX" &>/dev/null; then
    ...
    for MODE in debug release; do
        ...
        if $CLANG_CXX $COMMON_FLAGS $MODE_FLAGS "$TEST_FILE" -o "$OUTPUT" 2>&1; then
            if [[ -f "$OUTPUT" ]]; then
...
if ! command -v "$INTEL_CXX" &>/dev/null; then
    ...
    for MODE in debug release; do
        ...
        if $INTEL_CXX $COMMON_FLAGS $MODE_FLAGS "$TEST_FILE" -o "$OUTPUT" 2>&1; then
            if [[ -f "$OUTPUT" ]]; then
...
if ! command -v "$MSVC_CL" &>/dev/null; then
    ...
    for MODE in debug release; do
        ...
        OUTPUT="$OUTPUT_DIR/${TYPE}_${FEATURE}_tests_${COMPILER}.exe"
        ...
        if $MSVC_CL $COMMON_FLAGS $MODE_FLAGS "$TEST_FILE" /Fe:"$OUTPUT" /Fo:"$OUTPUT_DIR/" 2>&1; then
...
echo ""
echo_success "Build complete for uint128 algorithm tests!"
