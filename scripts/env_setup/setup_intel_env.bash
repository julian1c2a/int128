...
if [ "${BASH_SOURCE[0]}" -ef "$0" ]; then
    echo -e "${RED}[ERROR] Error: Este script debe ejecutarse con 'source'${NC}"
    ...
    if [ -f "$SCRIPT_DIR/setvarsall_intel.py" ]; then
        ...
        if [ $? -eq 0 ] && [ -s "$CACHE_FILE" ]; then
            ...
            if grep -q "^export" "$CACHE_FILE"; then
                echo -e "${GREEN}[OK] Cache generado exitosamente${NC}"
                return 0
            else
                rm -f "$CACHE_FILE"
                echo -e "${RED}[ERROR] Cache inválido (no contiene exports)${NC}"
                return 1
            fi
        else
            rm -f "$CACHE_FILE"
            echo -e "${RED}[ERROR] Error al generar cache${NC}"
            [ -f "$SCRIPT_DIR/.intel_setup.log" ] && echo -e "${YELLOW}   Ver detalles en: $SCRIPT_DIR/.intel_setup.log${NC}"
            return 1
        fi
    else
        echo -e "${RED}[ERROR] No se encontró setvarsall_intel.py${NC}"
        return 1
    fi
}
...
if [ -f "$CACHE_FILE" ] && [ -s "$CACHE_FILE" ]; then
    ...
    if [ -n "$ICPX_PATH" ]; then
        echo -e "${GREEN}[OK] Intel C++ compiler (icpx) encontrado en PATH${NC}"
        echo -e "   Ubicación: $ICPX_PATH"
    elif [ -n "$ICX_PATH" ]; then
        echo -e "${GREEN}[OK] Intel C++ compiler (icx) encontrado en PATH${NC}"
    ...
    if [ -n "$INCLUDE" ] && [ -n "$LIB" ]; then
        echo -e "${GREEN}[OK] Variables INCLUDE y LIB configuradas${NC}"
    ...
    echo -e "${RED}[ERROR] Error: No se pudo configurar el entorno Intel${NC}"
