...
if [ "$COMPILER" = "all" ] || [ "$COMPILER" = "gcc" ]; then
    ...
    mkdir -p "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/gcc/debug"
    mkdir -p "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/gcc/release"

    if command -v g++ &> /dev/null; then
        g++ "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -g -O0 \
            -o "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/gcc/debug/uint128_concepts_extracted_benchs.exe"
        echo "   ✅ GCC Debug OK"

        g++ "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -O3 -DNDEBUG \
            -o "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/gcc/release/uint128_concepts_extracted_benchs.exe"
        echo "   ✅ GCC Release OK"
    else
...
if [ "$COMPILER" = "all" ] || [ "$COMPILER" = "clang" ]; then
    ...
    mkdir -p "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/clang/debug"
    mkdir -p "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/clang/release"

    if command -v clang++ &> /dev/null; then
        clang++ "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -g -O0 \
            -o "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/clang/debug/uint128_concepts_extracted_benchs.exe"
        echo "   ✅ Clang Debug OK"

        clang++ "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -O3 -DNDEBUG \
            -o "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/clang/release/uint128_concepts_extracted_benchs.exe"
...
if [ "$COMPILER" = "all" ] || [ "$COMPILER" = "intel" ]; then
    ...
    if ! command -v icx &> /dev/null; then
        ...
        mkdir -p "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/intel/debug"
        mkdir -p "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/intel/release"
        
        # Convertir rutas a formato Windows
        WIN_SOURCE=$(cygpath -m "$SOURCE_FILE")
        WIN_INCLUDE=$(cygpath -m "$PROJECT_ROOT/include")
        WIN_OUT_DEBUG=$(cygpath -m "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/intel/debug/uint128_concepts_extracted_benchs.exe")
        WIN_OUT_RELEASE=$(cygpath -m "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/intel/release/uint128_concepts_extracted_benchs.exe")
...
if [ "$COMPILER" = "all" ] || [ "$COMPILER" = "msvc" ]; then
    ...
    if ! command -v cl &> /dev/null; then
        ...
        mkdir -p "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/msvc/debug"
        mkdir -p "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/msvc/release"
        
        WIN_SOURCE=$(cygpath -m "$SOURCE_FILE")
        WIN_INCLUDE=$(cygpath -m "$PROJECT_ROOT/include")
        WIN_OUT_DEBUG=$(cygpath -m "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/msvc/debug/uint128_concepts_extracted_benchs.exe")
        WIN_OUT_RELEASE=$(cygpath -m "$PROJECT_ROOT/build/uint128_concepts_extracted_benchs/msvc/release/uint128_concepts_extracted_benchs.exe")
