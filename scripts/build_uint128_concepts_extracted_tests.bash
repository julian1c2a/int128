    ...
    if ! command -v g++ &> /dev/null; then
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "debug" ]; then
            ...
            g++ "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -g -O0 \
                -o "$PROJECT_ROOT/build/build_tests/gcc/debug/uint128_concepts_extracted_tests.exe"
            echo "   ✅ GCC Debug OK"
        fi
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "release" ]; then
            ...
            g++ "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -O3 -DNDEBUG \
                -o "$PROJECT_ROOT/build/build_tests/gcc/release/uint128_concepts_extracted_tests.exe"
            echo "   ✅ GCC Release OK"
        fi
    fi
fi
    ...
    if ! command -v clang++ &> /dev/null; then
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "debug" ]; then
            ...
            clang++ "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -g -O0 \
                -o "$PROJECT_ROOT/build/build_tests/clang/debug/uint128_concepts_extracted_tests.exe"
            echo "   ✅ Clang Debug OK"
        fi
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "release" ]; then
            ...
            clang++ "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -O3 -DNDEBUG \
                -o "$PROJECT_ROOT/build/build_tests/clang/release/uint128_concepts_extracted_tests.exe"
...
if [ "$COMPILER" = "all" ] || [ "$COMPILER" = "intel" ]; then
    ...
    if ! command -v "$INTEL_CMD" &> /dev/null; then
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "debug" ]; then
            ...
            WIN_OUT_DEBUG=$(cygpath -m "$PROJECT_ROOT/build/build_tests/intel/debug/uint128_concepts_extracted_tests.exe")
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "release" ]; then
            ...
            WIN_OUT_RELEASE=$(cygpath -m "$PROJECT_ROOT/build/build_tests/intel/release/uint128_concepts_extracted_tests.exe")
...
if [ "$COMPILER" = "all" ] || [ "$COMPILER" = "msvc" ]; then
    ...
    if ! command -v cl &> /dev/null; then
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "debug" ]; then
            ...
            WIN_OUT_DEBUG=$(cygpath -m "$PROJECT_ROOT/build/build_tests/msvc/debug/uint128_concepts_extracted_tests.exe")
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "release" ]; then
            ...
            WIN_OUT_RELEASE=$(cygpath -m "$PROJECT_ROOT/build/build_tests/msvc/release/uint128_concepts_extracted_tests.exe")
