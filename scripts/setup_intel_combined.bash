...
if [ ! -f "$PYTHON_SCRIPT" ]; then
    echo "❌ ERROR FATAL: No se encuentra el archivo Python:"
    echo "   $PYTHON_SCRIPT"
...
if command -v python3 &> /dev/null; then
    ...
    echo "❌ ERROR FATAL: Python no encontrado en el PATH."
...
if [ $RET_CODE -ne 0 ]; then
    echo "❌ ERROR: El script de Python falló (Código $RET_CODE)."
...
if [ -s "$TEMP_ENV_FILE" ]; then
    ...
    echo "❌ ERROR: El script de Python no generó salida."
    cat "$LOG_FILE"
fi
rm -f "$TEMP_ENV_FILE"
...
if command -v icx &> /dev/null; then
    echo "✅ ÉXITO: Entorno cargado."
    echo "   Compilador: $(which icx)"
    # Check rápido de visual studio
    if [[ "$INCLUDE" == *"Visual Studio"* ]]; then
        echo "   MSVC Headers: DETECTADOS"
    else
        echo "⚠️  ADVERTENCIA: Headers de MSVC no detectados en INCLUDE."
    fi
else
    echo "❌ ERROR: 'icx' no está en el PATH."
fi
echo "---------------------------------------------------------"