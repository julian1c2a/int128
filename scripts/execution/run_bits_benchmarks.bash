#!/usr/bin/env bash
# Script para ejecutar benchmarks de bits con diferentes compiladores
...
echo -e "${COLOR_BLUE}  Run Bits Benchmarks${COLOR_RESET}"
echo -e "${COLOR_BLUE}========================================${COLOR_RESET}"
...
run_uint128_benchmark() {
    ...
    local bench_exe="${BUILD_DIR}/${compiler}/${mode}/uint128_bits_benchmarks.exe"
    ...
    echo -e "\n${COLOR_CYAN}[RUN] uint128_bits_benchmarks (${compiler}, ${mode})${COLOR_RESET}"
...
run_int128_benchmark() {
    local compiler="${1}"
    local mode="${2:-release}"
    local bench_exe="${BUILD_DIR}/${compiler}/${mode}/int128_bits_benchmarks.exe"
    ...
    echo -e "\n${COLOR_CYAN}[RUN] int128_bits_benchmarks (${compiler}, ${mode})${COLOR_RESET}"
...
main() {
    ...
    if [[ ${failed} -eq 0 ]]; then
        ...
        echo -e "${COLOR_YELLOW}[WARNING] ${failed} benchmark(s) no se ejecutaron${COLOR_RESET}"
        echo -e "${COLOR_BLUE}========================================${COLOR_RESET}"
        exit 0
    fi
}
