#!/usr/bin/env bash
# Script para compilar uint128_algorithm_benchmarks.cpp con diferentes compiladores
...
SOURCE_FILE="${PROJECT_ROOT}/benchmarks/uint128_algorithm_benchmarks.cpp"
...
echo -e "${COLOR_BLUE}  Build uint128_algorithm_benchmarks${COLOR_RESET}"
echo -e "${COLOR_BLUE}========================================${COLOR_RESET}"
...
build_gcc() {
    ...
    if g++ ${flags} "${SOURCE_FILE}" -o "${output_dir}/uint128_algorithm_benchmarks.exe"; then
        echo -e "${COLOR_GREEN}[OK] GCC compilacion exitosa${COLOR_RESET}"
        echo -e "${COLOR_GREEN}     Output: ${output_dir}/uint128_algorithm_benchmarks.exe${COLOR_RESET}"
        return 0
    else
...
build_clang() {
    ...
    if clang++ ${flags} "${SOURCE_FILE}" -o "${output_dir}/uint128_algorithm_benchmarks.exe"; then
...
build_msvc() {
    ...
    local win_output_file=$(cygpath -m "${output_dir}/uint128_algorithm_benchmarks.exe")
...
main() {
    ...
    case "${compiler}" in
        gcc)
            build_gcc "${mode}" || failed=$((failed + 1))
            ;;
        clang)
            build_clang "${mode}" || failed=$((failed + 1))
            ;;
        msvc)
            build_msvc "${mode}" || failed=$((failed + 1))
            ;;
        intel)
            build_intel "${mode}" || failed=$((failed + 1))
            ;;
        all)
            build_gcc "${mode}" || failed=$((failed + 1))
            build_clang "${mode}" || failed=$((failed + 1))
