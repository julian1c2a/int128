    ...
    if ! command -v clang++ &> /dev/null; then
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "debug" ]; then
            ...
            clang++ "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -g -O0 \
                -o "$PROJECT_ROOT/build/build_benchs/clang/debug/uint128_t_extracted_benchs.exe" 2>&1 | grep -v "LINK : warning LNK" | grep -v "^LINK : " || true
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "release" ]; then
            ...
            clang++ "$SOURCE_FILE" $INCLUDE_DIR $COMMON_FLAGS -O3 -DNDEBUG \
                -o "$PROJECT_ROOT/build/build_benchs/clang/release/uint128_t_extracted_benchs.exe" 2>&1 | grep -v "LINK : warning LNK" | grep -v "^LINK : " || true
