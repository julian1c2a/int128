#!/usr/bin/env bash
# Build script para int128_t_extracted_tests.cpp con 4 compiladores
...
# Mostrar timestamp
echo "========================================="
echo " BUILD: int128_t_extracted_tests"
...
if [ "$COMPILER" = "all" ] || [ "$COMPILER" = "intel" ]; then
    ...
    if ! command -v "$INTEL_CMD" &> /dev/null; then
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "debug" ]; then
            ...
            WIN_OUT_DEBUG=$(cygpath -m "$PROJECT_ROOT/build/build_tests/intel/debug/int128_t_extracted_tests.exe")
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "release" ]; then
            ...
            WIN_OUT_RELEASE=$(cygpath -m "$PROJECT_ROOT/build/build_tests/intel/release/int128_t_extracted_tests.exe")
...
if [ "$COMPILER" = "all" ] || [ "$COMPILER" = "msvc" ]; then
    ...
    if ! command -v cl.exe &> /dev/null; then
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "debug" ]; then
            ...
            WIN_OUT_DEBUG=$(cygpath -m "$PROJECT_ROOT/build/build_tests/msvc/debug/int128_t_extracted_tests.exe")
        ...
        if [ "$MODE" = "all" ] || [ "$MODE" = "release" ]; then
            ...
            WIN_OUT_RELEASE=$(cygpath -m "$PROJECT_ROOT/build/build_tests/msvc/release/int128_t_extracted_tests.exe")
