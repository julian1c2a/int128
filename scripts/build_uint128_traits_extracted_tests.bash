#!/bin/bash
# Script para compilar tests de uint128_traits
# Uso: ./build_uint128_traits_extracted_tests.bash [gcc|clang|intel|msvc|all] [debug|release]

set -euo pipefail

# Colores
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
CYAN='\033[0;36m'
NC='\033[0m'

# Directorios
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
PROJECT_DIR="${ROOT_DIR}"

# Validar argumentos
if [ $# -lt 1 ]; then
    echo -e "${RED}Error: Se requiere al menos un argumento${NC}"
    echo "Uso: $0 [gcc|clang|intel|msvc|all] [debug|release]"
    exit 1
fi

COMPILER="${1}"
COMPILER=$(echo "$COMPILER" | tr '[:upper:]' '[:lower:]')
MODE="${2:-release}"
MODE=$(echo "$MODE" | tr '[:upper:]' '[:lower:]')

# Validar modo
if [ "$MODE" != "debug" ] && [ "$MODE" != "release" ] && [ "$MODE" != "all" ]; then
    echo -e "${RED}Error: Modo inválido '$MODE'. Use 'debug', 'release' o 'all'${NC}"
    exit 1
fi

# Si MODE=all, compilar debug y release recursivamente
if [ "$MODE" = "all" ]; then
    echo -e "${BLUE}Compilando en modo 'all': debug + release${NC}"
    "$0" "$COMPILER" debug "${3:-}" && "$0" "$COMPILER" release "${3:-}"
    exit $?
fi

# Validar compilador
case "$COMPILER" in
    gcc|clang|intel|msvc|all)
        ;;
    *)
        echo -e "${RED}Error: Compilador inválido '$COMPILER'${NC}"
        echo "Uso: $0 [gcc|clang|intel|msvc|all] [debug|release]"
        exit 1
        ;;
esac

TIMESTAMP=$(date '+%Y-%m-%d %H:%M:%S')
echo -e "${BLUE}============================================================${NC}"
echo -e "${BLUE}  Compilación de uint128_traits tests${NC}"
echo -e "${BLUE}  Fecha: ${TIMESTAMP}${NC}"
echo -e "${BLUE}  Modo: ${MODE}${NC}"
echo -e "${BLUE}============================================================${NC}"

# Función auxiliar
should_compile() {
    local comp=$1
    [ "$COMPILER" = "all" ] || [ "$COMPILER" = "$comp" ]
}

# Crear directorios
mkdir -p build/build_tests/{gcc,clang,intel,msvc}/{debug,release}

# Archivos
TEST_SRC="tests/uint128_traits_extracted_tests.cpp"
INCLUDE_DIR="./include"

# -----------------------------------------------------------------------------
# GCC
# -----------------------------------------------------------------------------
build_gcc() {
    echo -e "\n${YELLOW}[GCC] Compilando tests...${NC}"
    
    if ! command -v g++ &> /dev/null; then
        echo -e "${RED}  [WARN]  g++ no encontrado en el PATH${NC}"
        return 1
    fi
    
    local FLAGS="-std=c++20 -Wall -Wextra -I${INCLUDE_DIR}"
    if [ "$MODE" = "debug" ]; then
        FLAGS="$FLAGS -g -O0"
    else
        FLAGS="$FLAGS -O3 -march=native"
    fi
    
    local OUTPUT="build/build_tests/gcc/${MODE}/uint128_traits_tests_gcc"
    
    g++ $FLAGS "${TEST_SRC}" -o "${OUTPUT}"
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}    [OK] Tests compilados OK${NC}"
        ls -lh --time-style='+%Y-%m-%d %H:%M:%S' "${OUTPUT}" | awk -v path="${OUTPUT}" '{print "    Ejecutable:", $6, $7, "-", path, "(" $5 ")"}'
        return 0
    else
        echo -e "${RED}    [FAIL] Error compilando tests${NC}"
        return 1
    fi
}

# -----------------------------------------------------------------------------
# CLANG
# -----------------------------------------------------------------------------
build_clang() {
    echo -e "\n${YELLOW}[CLANG] Compilando tests...${NC}"
    
    if ! command -v clang++ &> /dev/null; then
        echo -e "${RED}  [WARN]  clang++ no encontrado en el PATH${NC}"
        return 1
    fi
    
    local FLAGS="-std=c++20 -Wall -Wextra -I${INCLUDE_DIR}"
    if [ "$MODE" = "debug" ]; then
        FLAGS="$FLAGS -g -O0"
    else
        FLAGS="$FLAGS -O3 -march=native"
    fi
    
    local OUTPUT="build/build_tests/clang/${MODE}/uint128_traits_tests_clang"
    
    echo -e "${CYAN}    Comando: clang++ $FLAGS ${TEST_SRC} -o ${OUTPUT}${NC}"
    clang++ $FLAGS "${TEST_SRC}" -o "${OUTPUT}"
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}    [OK] Tests compilados OK${NC}"
        ls -lh "${OUTPUT}" | awk '{print "    Ejecutable:", $9, "(" $5 ")"}'
        return 0
    else
        echo -e "${RED}    [FAIL] Error compilando tests${NC}"
        return 1
    fi
}

# -----------------------------------------------------------------------------
# INTEL ONEAPI (ICX)
# -----------------------------------------------------------------------------
build_intel() {
    echo -e "\n${YELLOW}[INTEL] Compilando tests con Intel OneAPI...${NC}"
    
    if ! command -v icx &> /dev/null; then
        echo -e "${RED}  [WARN]  Intel 'icx' no encontrado en el PATH${NC}"
        echo -e "      Ejecuta primero: source scripts/setup_intel_combined.bash"
        return 1
    fi
    
    # Flags estilo Windows para ICX
    local FLAGS="/std:c++20 /W3 /EHsc /Zc:__cplusplus -D_HAS_CXX20=1 -Xclang -fcxx-exceptions"
    if [ "$MODE" = "debug" ]; then
        FLAGS="$FLAGS /Od /Zi"
    else
        FLAGS="$FLAGS /O3 /QxHost"
    fi
    
    local OUTPUT="build/build_tests/intel/${MODE}/uint128_traits_tests_intel.exe"
    
    # IMPORTANTE: export MSYS2_ARG_CONV_EXCL="*" evita que /O3 se convierta en C:\msys64\O3
    export MSYS2_ARG_CONV_EXCL="*"
    
    icx $FLAGS \
        -I"${INCLUDE_DIR}" \
        tests/uint128_traits_extracted_tests.cpp \
        -o build/build_tests/intel/${MODE}/uint128_traits_tests_intel.exe
    
    local result=$?
    unset MSYS2_ARG_CONV_EXCL
    
    if [ $result -eq 0 ]; then
        echo -e "${GREEN}    [OK] Tests compilados OK${NC}"
        ls -lh --time-style='+%Y-%m-%d %H:%M:%S' "${OUTPUT}" | awk -v path="${OUTPUT}" '{print "    Ejecutable:", $6, $7, "-", path, "(" $5 ")"}'
        return 0
    else
        echo -e "${RED}    [FAIL] Error compilando tests${NC}"
        return 1
    fi
}

# -----------------------------------------------------------------------------
# MSVC
# -----------------------------------------------------------------------------
build_msvc() {
    echo -e "\n${YELLOW}[MSVC] Compilando tests...${NC}"
    
    # Intentar cl.exe primero, luego clang-cl como fallback
    local COMPILER=""
    if command -v cl.exe &> /dev/null; then
        COMPILER="cl.exe"
        echo "  Usando: cl.exe (MSVC nativo)"
    elif command -v clang-cl &> /dev/null; then
        COMPILER="clang-cl"
        echo "  Usando: clang-cl (modo compatible MSVC)"
    else
        echo -e "${RED}  [WARN]  Ni cl.exe ni clang-cl encontrados en el PATH${NC}"
        echo -e "      Ejecuta vcvarsall.bat o instala Clang"
        return 1
    fi
    
    local FLAGS="/std:c++20 /W4 /EHsc"
    if [ "$MODE" = "debug" ]; then
        FLAGS="$FLAGS /Od /Zi"
    else
        FLAGS="$FLAGS /O2 /GL /arch:AVX2"
    fi
    
    local OUTPUT="build/build_tests/msvc/${MODE}/uint128_traits_tests_msvc.exe"
    
    # Desactivar conversión de rutas de MSYS2 para MSVC
    export MSYS2_ARG_CONV_EXCL="*"
    
    $COMPILER $FLAGS \
        /I"${INCLUDE_DIR}" \
        "${TEST_SRC}" \
        /Fe"${OUTPUT}" \
        > /dev/null 2>&1
    
    local result=$?
    unset MSYS2_ARG_CONV_EXCL
    
    if [ $result -eq 0 ]; then
        echo -e "${GREEN}    [OK] Tests compilados OK${NC}"
        # Limpiar archivos temporales
        rm -f build/build_tests/msvc/${MODE}/*.obj build/build_tests/msvc/${MODE}/*.pdb
        ls -lh --time-style='+%Y-%m-%d %H:%M:%S' "${OUTPUT}" | awk -v path="${OUTPUT}" '{print "    Ejecutable:", $6, $7, "-", path, "(" $5 ")"}'
        return 0
    else
        echo -e "${RED}    [FAIL] Error compilando tests${NC}"
        return 1
    fi
}

# -----------------------------------------------------------------------------
# MAIN
# -----------------------------------------------------------------------------
main() {
    local success=0
    local failed=0
    
    if should_compile "gcc"; then
        if build_gcc; then
            ((success++)) || true
        else
            ((failed++)) || true
        fi
    fi
    
    if should_compile "clang"; then
        if build_clang; then
            ((success++)) || true
        else
            ((failed++)) || true
        fi
    fi
    
    if should_compile "intel"; then
        if build_intel; then
            ((success++)) || true
        else
            ((failed++)) || true
        fi
    fi
    
    if should_compile "msvc"; then
        if build_msvc; then
            ((success++)) || true
        else
            ((failed++)) || true
        fi
    fi
    
    echo -e "\n${BLUE}============================================================${NC}"
    echo -e "${BLUE}  Resumen de compilación${NC}"
    echo -e "${BLUE}============================================================${NC}"
    echo -e "${GREEN}  Exitosos: ${success}${NC}"
    echo -e "${RED}  Fallidos: ${failed}${NC}"
    echo -e "${BLUE}============================================================${NC}"
    
    return ${failed}
}

main
exit $?

