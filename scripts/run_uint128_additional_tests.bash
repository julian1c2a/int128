...
if [ -f "build/build_tests/gcc/release/test_uint128_additional.exe" ]; then
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✓ Tests GCC pasaron exitosamente${NC}"
    else
        echo -e "${RED}✗ Tests GCC fallaron${NC}"
    fi
    echo ""
fi
...
if [ -f "build/build_tests/clang/release/uint128_additional_tests.exe" ]; then
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✓ Tests Clang pasaron exitosamente${NC}"
    else
        echo -e "${RED}✗ Tests Clang fallaron${NC}"
    fi
    echo ""
fi
...
if [ -f "build/build_tests/msvc/release/uint128_additional_tests.exe" ]; then
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✓ Tests MSVC pasaron exitosamente${NC}"
    else
        echo -e "${RED}✗ Tests MSVC fallaron${NC}"
    fi
    echo ""
fi
