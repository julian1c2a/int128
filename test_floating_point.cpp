...
void test_uint128_constructor_float()
{
    ...
    std::cout << "✓ Constructor desde float pequeño: 42.0f\n";
...
void test_uint128_constructor_double()
{
    ...
    assert(val1.high() == 0ull);
    std::cout << "✓ Constructor desde double con decimales: 123.456 → 123\n";

    // Test 2: Valor muy grande
    const double huge = 18446744073709551616.0 * 1000.0; // 2^64 * 1000
    uint128_t val2(huge);
    assert(val2.high() == 1000ull);
    std::cout << "✓ Constructor desde double muy grande: 2^64 * 1000\n";
}
...
void test_uint128_assignment_float()
{
    ...
    std::cout << "✓ Asignación desde float pequeño: 100.0f\n";

    // Test 2: Asignar valor grande
    val = 18446744073709551616.0f * 5.0f;
    assert(val.high() == 5ull);
    std::cout << "✓ Asignación desde float grande: 2^64 * 5\n";

    // Test 3: Valor negativo → cero
    val = -50.0f;
    assert(val.low() == 0ull);
    assert(val.high() == 0ull);
    std::cout << "✓ Asignación desde float negativo → 0\n";
}
...
void test_int128_constructor_float()
{
    ...
    assert(val1.low() == 42ull);
    assert(val1.high() == 0ull);
    std::cout << "✓ Constructor int128 desde float positivo: 42.0f\n";
    ...
    std::cout << "✓ Constructor int128 desde float negativo: -100.0f\n";

    // Test 3: Valor grande positivo
    const float big = 18446744073709551616.0f * 2.0f;
    int128_t val3(big);
    assert(val3.high() == 2ull);
    std::cout << "✓ Constructor int128 desde float grande: 2^65\n";
}
...
void test_int128_constructor_double()
{
    ...
    assert(val1.high() == 0ull);
    std::cout << "✓ Constructor int128 desde double: 456.789 → 456\n";

    // Test 2: Valor negativo con decimales
    int128_t val2(-999.123);
    assert(val2.is_negative());
    std::cout << "✓ Constructor int128 desde double negativo: -999.123\n";
}
...
void test_int128_assignment_double()
{
    ...
    assert(val.high() == 0ull);
    std::cout << "✓ Asignación int128 desde double positivo: 1234.0\n";

    // Test 2: Asignar negativo
    val = -5678.0;
    assert(val.is_negative());
    std::cout << "✓ Asignación int128 desde double negativo: -5678.0\n";

    // Test 3: Asignar grande
    val = 18446744073709551616.0 * 10.0;
    assert(val.high() == 10ull);
    std::cout << "✓ Asignación int128 desde double grande: 2^64 * 10\n";
}
...
void test_conversion_to_float()
{
    ...
    std::cout << "✓ Conversión uint128_t grande → double: 10 * 2^64\n";
}
...
void test_roundtrip()
{
    ...
    std::cout << "✓ Roundtrip float → uint128_t → float: 12345.0f\n";
    ...
    std::cout << "✓ Roundtrip double → int128_t → double (positivo): 98765.0\n";
    ...
    std::cout << "✓ Roundtrip double → int128_t → double (negativo): -54321.0\n";
}
...
void test_explicit_requirement()
{
    ...
    // uint128_t implicit1 = 42.0f;        // ✗ Error: constructor explicit
    // int128_t implicit2 = -100.0;        // ✗ Error: constructor explicit

    // Estas SÍ deben compilar (cast explícito):
    uint128_t explicit1(42.0f);             // ✓ OK
    int128_t explicit2(-100.0);             // ✓ OK
    uint128_t explicit3 = uint128_t(3.14f); // ✓ OK

    std::cout << "✓ Constructores requieren cast explícito (regla explicit)\n";
}
