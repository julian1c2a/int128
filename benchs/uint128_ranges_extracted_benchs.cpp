...
void benchmark_iota()
{
    std::vector<uint128_t> vec(SIZE);

    auto start = high_resolution_clock::now();
    std::iota(vec.begin(), vec.end(), uint128_t(0, 0));
...
{
    std::vector<uint128_t> vec(SIZE);
    std::iota(vec.begin(), vec.end(), uint128_t(0, 1));
...
{
    std::vector<uint128_t> vec1(SIZE);
    std::vector<uint128_t> vec2(SIZE);
    std::iota(vec1.begin(), vec1.end(), uint128_t(0, 1));
    std::iota(vec2.begin(), vec2.end(), uint128_t(0, 2));
...
void benchmark_partial_sum()
{
    std::vector<uint128_t> vec(SIZE);
    std::iota(vec.begin(), vec.end(), uint128_t(0, 1));
    std::vector<uint128_t> result(SIZE);
...
{
    std::vector<uint128_t> vec(SIZE);
    std::iota(vec.begin(), vec.end(), uint128_t(0, 1000));
    std::vector<uint128_t> result(SIZE);
...
int main()
{
    std::cout << "uint128_t ranges benchmarks" << std::endl;
    std::cout << "===========================" << std::endl;
