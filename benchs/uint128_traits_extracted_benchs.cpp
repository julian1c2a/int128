/*
 * Benchmarks para uint128_traits.hpp
...
#define UINT128_TRAITS_NOT_AVAILABLE 1
#else
#define UINT128_TRAITS_NOT_AVAILABLE 0
#endif
...
void benchmark_traits_queries()
{
    ...
    // is_integral
    benchmark("is_integral<uint128_t>",
              []() { [[maybe_unused]] bool v = std::is_integral_v<uint128_t>; });

    // is_unsigned
    benchmark("is_unsigned<uint128_t>",
              []() { [[maybe_unused]] bool v = std::is_unsigned_v<uint128_t>; });

    // is_arithmetic
    benchmark("is_arithmetic<uint128_t>",
              []() { [[maybe_unused]] bool v = std::is_arithmetic_v<uint128_t>; });

    // make_unsigned
    benchmark("make_unsigned<uint128_t>",
              []() { [[maybe_unused]] typename std::make_unsigned<uint128_t>::type v; });

    // numeric_limits::max
    benchmark("numeric_limits<uint128_t>::max",
              []() { [[maybe_unused]] auto v = std::numeric_limits<uint128_t>::max(); });

    // numeric_limits::min
    benchmark("numeric_limits<uint128_t>::min",
              []() { [[maybe_unused]] auto v = std::numeric_limits<uint128_t>::min(); });

    // numeric_limits::digits
    benchmark("numeric_limits<uint128_t>::digits",
              []() { [[maybe_unused]] auto v = std::numeric_limits<uint128_t>::digits; });
#else
...
void benchmark_container_usage()
{
    ...
    benchmark(
        "vector<uint128_t>::accumulate",
        [&]() {
            [[maybe_unused]] auto sum = std::accumulate(vec.begin(), vec.end(), uint128_t(0));
        },
        10000);
#else
    std::cout << "  [SKIP] Traits no soportados en esta configuración STL\n";
...
{
    std::cout << "========================================\n";
    std::cout << "uint128_t Traits Benchmarks\n";
    std::cout << "========================================\n";
