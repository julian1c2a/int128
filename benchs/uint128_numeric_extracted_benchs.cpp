/**
 * @file uint128_numeric_benchmarks.cpp
 * @brief Performance benchmarks for uint128_numeric.hpp functions
 *
 * Benchmarks C++20 numeric functions:
 * - std::midpoint (safe midpoint calculation)
 * - std::clamp (range restriction)
...
{
    std::vector<uint128_t> values_a(ITERATIONS);
    std::vector<uint128_t> values_b(ITERATIONS);
    std::vector<uint128_t> results(ITERATIONS);

    // Generate test data
    std::mt19937_64 rng(42);
    std::uniform_int_distribution<uint64_t> dist(1000, UINT64_MAX - 1000);

    for (size_t i = 0; i < ITERATIONS; ++i) {
        values_a[i] = uint128_t(dist(rng));
        values_b[i] = uint128_t(dist(rng));
    }
    ...
    volatile uint128_t sink = results[ITERATIONS / 2];
    (void)sink;
...
{
    std::vector<uint128_t> values(ITERATIONS);
    std::vector<uint128_t> results(ITERATIONS);
    uint128_t lo = uint128_t(100);
    uint128_t hi = uint128_t(1000);

    std::mt19937_64 rng(42);
    std::uniform_int_distribution<uint64_t> dist(0, 2000);

    for (size_t i = 0; i < ITERATIONS; ++i) {
        values[i] = uint128_t(dist(rng));
    }
...
BenchmarkResult bench_popcount()
{
    ...
    for (size_t i = 0; i < ITERATIONS; ++i) {
        values[i] = uint128_t(dist(rng), dist(rng));
    }
...
BenchmarkResult bench_countr_zero()
{
    ...
    for (size_t i = 0; i < ITERATIONS; ++i) {
        values[i] = uint128_t(dist(rng)) << (i % 64);
    }
...
BenchmarkResult bench_has_single_bit()
{
    ...
    for (size_t i = 0; i < ITERATIONS; ++i) {
        if (i % 2 == 0) {
            values[i] = uint128_t(1) << (i % 128); // Power of 2
        } else {
            values[i] = uint128_t(dist(rng));
        }
    }
...
int main()
{
    std::cout << "========================================================================"
              << std::endl;
    std::cout << "    UINT128_T NUMERIC FUNCTIONS BENCHMARK" << std::endl;
    std::cout << "========================================================================"
              << std::endl;
    ...
    std::cout << "[1/9] Benchmarking midpoint..." << std::endl;
    results.push_back(bench_midpoint());

    std::cout << "[2/9] Benchmarking clamp..." << std::endl;
    results.push_back(bench_clamp());

    std::cout << "[3/9] Benchmarking popcount..." << std::endl;
    results.push_back(bench_popcount());

    std::cout << "[4/9] Benchmarking countl_zero..." << std::endl;
    results.push_back(bench_countl_zero());

    std::cout << "[5/9] Benchmarking countr_zero..." << std::endl;
    results.push_back(bench_countr_zero());

    std::cout << "[6/9] Benchmarking bit_width..." << std::endl;
    results.push_back(bench_bit_width());

    std::cout << "[7/9] Benchmarking has_single_bit..." << std::endl;
    results.push_back(bench_has_single_bit());

    std::cout << "[8/9] Benchmarking bit_ceil..." << std::endl;
    results.push_back(bench_bit_ceil());

    std::cout << "[9/9] Benchmarking bit_floor..." << std::endl;
